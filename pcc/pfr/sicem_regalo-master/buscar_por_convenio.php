...
if ($busqueda<>''){
	...
	if ($numero==1) {
		...
		$cadbusca="SELECT C.* , D.* , S.* FROM convenio C, difunto D,solicitante S WHERE C.id_dif=D.id_dif AND S.id_sol=D.id_sol AND (C.dni_sol LIKE '%$busqueda%' OR S.nom_sol LIKE '%$busqueda%' OR D.nom_dif LIKE '%$busqueda%') ;";
		
		/*$cadbusca="SELECT C.* , D.* , S.* FROM convenio C, difunto D,solicitante S WHERE C.dni_sol LIKE '%$busqueda%' OR S.nom_sol LIKE '%$busqueda%' OR D.nom_dif LIKE '%$busqueda%' AND  C.dni_sol=S.dni_sol AND S.dni_sol=D.dni_sol;";*/
