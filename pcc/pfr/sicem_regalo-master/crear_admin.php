	...
	$cont=1;
	 for($i=1; $i<=$xtot_fil ; $i++){
		...
		for($j=1; $j<=$xtot_col ; $j++){
			...
			echo "<td align='center' width='110px' height='90px'><form action=nicho_editar_nuevo.php method=post> 
	...
	$fila=$xtot_fil;
	 for($i=1; $i<=$xtot_fil ; $i++){
		...
		for($j=1; $j<=$xtot_col ; $j++){
			...
			echo "<td align='center' width='110px' height='90px'><form action=nicho_editar.php method=post> 
							<input type=submit style='width:94px; height:87px; background:#32CD32' name=xnro_nicho value=".$cont.">
							<input type=hidden name=xid_pab value=".$xid_pab.">
						</form>
				</td>";  
